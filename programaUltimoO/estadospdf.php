...
$pdf->SetWidths(array(30, 40, 40, 30)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetros.  
$pdf->Row(array('Codigo', 'Personal', 'Accion','fecha')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita.  
$historial = $con->conectar(); // Creamos nuestra conexión a la base de datos  
 // Realizamos nuestra consulta  
 $laboratorio=$_POST['nombre'];
$strConsulta = "SELECT * from controles inner join equipos on controles.codigoequipo = equipos.codigoequipo where equipos.ubicacion='$laboratorio' order by controles.codigoequipo";  
...
$pdf->Row(array($fila['codigoequipo'], $fila['personal'], $fila['accion'],$fila['fecha']));  
}  
$pdf->Ln(5);
