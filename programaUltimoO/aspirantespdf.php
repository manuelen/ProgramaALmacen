...
$con = new DB; 
	$aspirante=$_POST['cedula'];
	$consulta=mysql_query("select cedula,nombre,apellido,telefono,curso from aspirantes where cedula='$cedula'");
...
$pdf->SetWidths(array(30, 40, 40, 30,30)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetros.  
$pdf->Row(array('Cédula', 'Nombre', 'Apellido', 'Teléfono','Curso')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita.  
$historial = $con->conectar(); // Creamos nuestra conexión a la base de datos  
 // Realizamos nuestra consulta  
 $aspirante=$_POST['cedula'];
$strConsulta = "SELECT * from aspirantes where cedula='$cedula'";  
