...
{  	
	...
	$this->Cell(250,10,'DOTACION DE EQUIPOS HERRAMIENTAS Y ACCESORIOS',0,0,'C');
	$this->Ln(10);
	$this->Cell(20);
...
$pdf->SetWidths(array(20, 20, 30, 30,15,50,18,30,30)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetros.  
$pdf->Row(array('Codigo', 'Nombre', 'Tipo', 'Marca','Serial','Descripcion','Cantidad','Estado')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita.  
$historial = $con->conectar(); // Creamos nuestra conexión a la base de datos  
 // Realizamos nuestra consulta  
 $laboratorio=$_POST['nombre'];
$strConsulta = "SELECT * from equipos where ubicacion='$laboratorio'";  
...
$pdf->Row(array($fila['codigoequipo'], $fila['nombre'], $fila['tipo'], $fila['marca'],$fila['serial'],$fila['descripcion'],$fila['cantidad'],$fila['estado']));  
}  
$pdf->Ln(5);
